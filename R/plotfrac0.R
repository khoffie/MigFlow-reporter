plot_frac0 <- function(dt) {
  ...
  plot_frac0_dist <- function(dt) {
    flows <- distance <- frac0 <- . <- NULL
    ggplot2::ggplot(dt[, .(frac0 = mean(flows == 0)), keyby = distance],
                    aes(distance, frac0)) +
  ...
  plot_frac0_dens <- function(dt) {
    flows <- frac0 <- fromdens <- todens <- . <- NULL
    ggplot2::ggplot(dt[, .(frac0 = mean(flows == 0)),
                       keyby = log(fromdens * todens)],
                    aes(log, frac0)) +
      geom_smooth(se = FALSE, linewidth = 2) +
      theme_bw() +
      ylab("") +
